...
FlashHelper.writeFlash = function() {
    ...
    else
    {
        ...
        document.write('<object id="storage" style="position:absolute;" data="' + swfName + '"');
...
function startApp(fs) {
    if (!fs) { alert("请确认该浏览器是否已安装flash插件！！"); return; }
    //fs.Debug();
}


//AMSCDR
function amsCDR(url, method, formname, callback, flowId) {
	var flowCdrStr = 'flowCdr_'+flowId+'= new amsAjaxCdr(flowId)';
	var flowCdrSubmitStr = 'flowCdr_'+flowId+'.AjaxCrossDomainRequest(url, method, formname, callback, flowId)';
	eval(flowCdrStr);
	eval(flowCdrSubmitStr);	
}


function amsAjaxCdr(flowId) {
	this.AjaxCrossDomainResponse = "";
	this.AjaxCrossDomainResname = "";
	this.flowId = flowId;
}
amsAjaxCdr.prototype.displayResponse = function() {
	this.AjaxCrossDomainResponse = FlashHelper.getFlash().GetVariable("retText");
	var sData = this.AjaxCrossDomainResponse;
	if (this.AjaxCrossDomainResponse){
		var ajaxCallBack = this.AjaxCrossDomainResname + '(sData)';
		//alert(ajaxCallBack);
	    eval(ajaxCallBack);
	}
}
amsAjaxCdr.prototype.AjaxCrossDomainRequest = function (url, method, formname, callback) {
	var self = this;
    method = js_strtoupper(method);	
    //alert('获取回调'+callback);
	this.AjaxCrossDomainResname = callback;
    ...
    var fs = FlashHelper.getFlash();
    FlashHelper.clUrl = url;
    FlashHelper.clMethod = method;
    FlashHelper.clFormname = formname;
    FlashHelper.clCallback = callback;
    FlashHelper.clBody = body;
    //callurla(fs);
    
	if (!fs.XmlHttp) {
		var storageOnLoadOld = storageOnLoad;
...
amsAjaxCdr.prototype.callurlb = function (fs) {
	var url = FlashHelper.clUrl;
	var method = FlashHelper.clMethod;
	var formname = FlashHelper.clFormname;
	var callback = FlashHelper.clCallback;
	var body =  FlashHelper.clBody;
	...
	var contentType = "application/x-www-form-urlencoded";
	var flowCdrStr = 'flowCdr_'+this.flowId;
	var disRes = flowCdrStr+'.displayResponse';
	if (method == "GET"){
        ...
        fs.XmlHttp(urlget, disRes, method, "", contentType);
	} else {
			
        fs.XmlHttp(url, disRes, method, body, contentType);
	}	
}
...
/*  |xGv00|975cbc21bb92daa52d983dcf0d93988f */